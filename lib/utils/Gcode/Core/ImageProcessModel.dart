...
import '../Engine/GCodeGeneration/BaseGCode.dart';
import '../Engine/GCodeGeneration/ImageProcessor/GCodeGenerator.dart';
import '../Engine/GCodeGeneration/ImageProcessor/StrokesFromImageLinesGenerator.dart';

import '../Engine/Interpolators/BilinearInterpolator.dart';
import '../Engine/Interpolators/StepInterpolator.dart';
import 'package:path_provider/path_provider.dart';
import 'package:sprintf/sprintf.dart';

import '../Engine/ImageProcessor.dart';
...
class ImageProcessorModel {
  ...
  ui.Image? get pickedImage => _pickedImage;
  ...
  Future openImage() async {
    try {
      _bitmap.image = await _bitmap.fromDevice();
      _pickedImage = _bitmap.image;
  ...
  Future saveImageAndGen(ByteData byteData) async {
    try {
      _bitmap.image = await _bitmap.fromByteData(byteData);
      aspectRate = _bitmap.width.toDouble() / _bitmap.height.toDouble();
    } catch (e) {
      print(e);
    }
  ...
  void generate() async {
    ...
    final file = File('/storage/emulated/0/Download/myFile.ngc');
    ...
    print('.ngc file save successfully');

    /*  try {
      var res = await http.post(Uri.parse("http://10.0.2.2:3000/saveData"),
